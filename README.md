...
5. **With custom configuration** (edit `docker-compose.yml`):
   ```yaml
   services:
     pubsub:
       environment:
...
Connect to `ws://localhost:8081/ws` for real-time messaging.
...
**Delete a topic**:
```bash
curl -X DELETE http://localhost:8081/topics/orders
```
